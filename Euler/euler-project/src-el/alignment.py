...
class TaxonomyMapping:
    ...
    def __init__(self, options):
        ...
        if reasoner[self.options.reasoner] == reasoner["gringo"]:
            ...
            # possible world command
            self.com = "/var/www/html/Euler/binary/dlv -silent -filter=rel "+self.pwfile+" "+ self.pwswitch+ " | "+self.path+"/muniq -u"
            # consistency command
            self.con = "/var/www/html/Euler/binary/dlv -silent -filter=rel -n=1 "+self.pwfile+" "+ self.pwswitch
        else:
...
## NF ends
    ...
    def outPW(self, name, pws, pwflag, ss):
        ...
        commands.getoutput("dot -Tsvg "+self.options.outputdir+self.name+"_all.dot -o "+self.options.outputdir+self.name+"_all.svg")
...
#        commands.getoutput("dot -Tpdf "+self.options.outputdir+fileName+".dot -o "+self.options.outputdir+fileName+".pdf")
        ...
        #if self.options.outputdir+fileName+".yaml":
	#	os.chmod(self.options.outputdir+fileName+".yaml", 777)
	fRcgVizYaml = open(self.options.outputdir+fileName+".yaml", 'w')
        ...
        commands.getoutput("cat "+self.options.outputdir+fileName+".yaml"+" | /var/www/html/Euler/euler-project/src-el/y2d -s "+self.stylesheetdir+"rcgstyle.yaml" + ">" + self.options.outputdir+fileName+".dot")
        commands.getoutput("dot -Tpdf "+self.options.outputdir+fileName+".dot -o "+self.options.outputdir+fileName+".pdf")
        commands.getoutput("dot -Tsvg "+self.options.outputdir+fileName+".dot -o "+self.options.outputdir+fileName+".svg")
    ...
    def genCB(self):
        ...
        if reasoner[self.options.reasoner] == reasoner["gringo"]:
            ...
            path = os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe())))
            self.com = "/var/www/html/Euler/binary/dlv -silent -filter=relout "+self.cbfile+" "+ self.pwswitch + " | "+path+"/muniq -u"
            self.cb = commands.getoutput(self.com)
            if self.isCbNone():
                self.remedy()
            if self.cb.find("error") != -1:
    ...
    def inputVisualization(self, group2concepts, art):
        ...
        # create the yaml file
        #if self.options.outputdir+self.name+".yaml":
	#	os.chmod(self.options.outputdir+self.name+".yaml", 777)
	fInputVizYaml = open(self.options.outputdir+self.name+".yaml", 'w')
        ...
        commands.getoutput("cat "+self.options.outputdir+self.name+".yaml"+" | /var/www/html/Euler/euler-project/src-el/y2d -s "+self.stylesheetdir+"inputstyle.yaml" + ">" + self.iv2dot)
